...
class BaseDataset(Dataset):
    ...
    def render(self, layout, img, border=2, W=None, H=None):
    ...
    def __getitem__(self, idx):   
        ...
        saliency = saliency.resize((self.W, self.H), Image.BILINEAR)
...
class WebUI(BaseDataset):
    ...
    def __init__(self, split, data_dir, max_length=None):    
        ...
        self.size = CANVAS_SIZE # pow(2, precision)
        ...
        if not os.path.exists(self.data_path):
            ...
            img_dir = os.path.join(data_dir, 'layout')
            ...
            for img_name in tqdm(dirs, desc='Processing dataset'):
                ...
                for ele in elements:
                    ...
                    if l + w - img_l > img_w or t + h - img_t > img_h:
                        continue
                    if (w * h) / (img_w * img_h) < 15 * 15 / (self.W * self.H):
                        continue
