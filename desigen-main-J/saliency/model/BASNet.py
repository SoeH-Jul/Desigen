...
class RefUnet(nn.Module):
    ...
    def execute(self,x):
        ...
        d4 = self.relu_d4(self.bn_d4(self.conv_d4(jt.concat((hx,hx4),1))))
        hx = self.upscore2(d4)

        d3 = self.relu_d3(self.bn_d3(self.conv_d3(jt.concat((hx,hx3),1))))
        hx = self.upscore2(d3)

        d2 = self.relu_d2(self.bn_d2(self.conv_d2(jt.concat((hx,hx2),1))))
        hx = self.upscore2(d2)

        d1 = self.relu_d1(self.bn_d1(self.conv_d1(jt.concat((hx,hx1),1))))
...
class BASNet(nn.Module):
    def __init__(self,n_channels,n_classes):
        ...
        resnet = jmodels.resnet34(pretrained=True)
    ...
    def execute(self,x):
        ...
        return jt.sigmoid(dout), jt.sigmoid(d1), jt.sigmoid(d2), jt.sigmoid(d3), jt.sigmoid(d4), jt.sigmoid(d5), jt.sigmoid(d6), jt.sigmoid(db)
