...
class Padding(object):
    ...
    def __call__(self, layout):
        ...
        chunk = jt.zeros((self.max_length+1,), dtype=jt.int32) + self.pad_token
...
class BaseDataset(Dataset):
    ...
    def render_normalized_layout(self, layout):
        ...
        layout = trim_tokens(jt.array(layout), self.bos_token, self.eos_token, self.pad_token).numpy()
    ...
    def __getitem__(self, idx):   
        ...
        layout = jt.array(self.data[idx], dtype=jt.int32)
        ...
        saliency = jt.array(np.asarray(saliency).astype("float32") / 255.0)
        if not img.mode == "RGB":
            img = img.convert("RGB")
        
        layout = self.pad(layout.flatten())
        img = jt.array(np.asarray(img).astype("float32") / 255.0)
...
class WebUI(BaseDataset):
    ...
    def save_pt(self, save_path):
        ...
        jt.save(results, os.path.join(os.path.dirname(save_path), self.processed_file_names[0]))
        ...
        jt.save(results, os.path.join(os.path.dirname(save_path), self.processed_file_names[1]))
        ...
        jt.save(results, os.path.join(os.path.dirname(save_path), self.processed_file_names[2]))
