...
def normalize(adj):
    ...
    mat1 = jt.pow(rowsum, -0.5).reshape(B, N, 1).repeat(1, 1, N) * jt.eye(N)
    mat2 = jt.pow(rowsum, -0.5).reshape(B, N, 1).repeat(1, 1, N) * jt.eye(N)
    adj_norm = mat1.bmm(adj_).bmm(mat2)
    return adj_norm

def top_k_logits(logits, k):
    v, ix = jt.topk(logits, k)
    out = logits.clone()
    ...
    with jt.no_grad():
        for k in range(steps):
            ...
            if only_label == True and k % 5 == 0:
                ...
                if sample:
                    ix = jt.multinomial(probs, num_samples=1)
                else:
                    _, ix = jt.topk(probs, k=1, dim=-1)
            # append to the sequence and continue
            x = jt.concat((x, ix), dim=1)
        return x


def trim_tokens(tokens, bos, eos, pad=None):
    bos_idx = jt.where(tokens == bos)[0]
    tokens = tokens[bos_idx[0]+1:] if len(bos_idx) > 0 else tokens
    eos_idx = jt.where(tokens == eos)[0]
...
def seq_to_bbox(layout_seq, bos, eos, pad):
    layout = trim_tokens(jt.array(layout_seq), bos, eos, pad).numpy()
...
def random_mask(adj, mask_ratio, dire_len=9):
    ...
    mask = jt.rand(adj.shape) < mask_ratio
    adj[mask] = dire_len + 1
    return adj

def random_noise(adj, noise_ratio, dire_len):
    # sample mask randomly from [0, 1)
    noise = jt.rand(adj.shape)
