...
def pil_to_jt_tensor(image, size=(224, 224)):
    image = image.resize(size)
    arr = np.asarray(image).astype("float32") / 255.0
    if arr.ndim == 2:
        arr = arr[:, :, None]
    arr = arr.transpose(2, 0, 1)
    return jt.unsqueeze(jt.array(arr), 0)
...
def get_evaler():
    ...
    encoder_model.load_state_dict(jt.load(args.encoder_path))
    decoder_model.load_state_dict(jt.load(args.decoder_path))
...
if jt.has_cuda:
    jt.flags.use_cuda = 1
    if hasattr(jt.flags, "cuda_device"):
        jt.flags.cuda_device = args.device
...
def max_mask(layouts):
    ...
    for layout in layouts:
        ...
        cur_sum = float(cur_mask.sum().item())
        if cur_sum > max_sum:
            mask = cur_mask
            max_sum = cur_sum
    return mask
...
def generate_design(prompt):
    ...
    for i in range(len(images)):
        image = images[i]
        image_tensor = pil_to_jt_tensor(image)
        # generate layout
...
def iterative_refine(prompt):
    ...
    mask = jt.unsqueeze(jt.zeros(mask_layout.shape), 0)
    
    for iter in range(iteration):
        mask = jt.logical_or(jt.unsqueeze(jt.array(mask_layout), 0), mask)
